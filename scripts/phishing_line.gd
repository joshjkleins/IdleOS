...
func begin(p_type):
	...
	add_heat()
	var eff_text = str((type["efficiency"] + Phishing.process_upgrades["efficiency"]["amount"] - 1.0) * 100).pad_decimals(1) 
	...
	if randf() <= type["efficiency"] + Phishing.process_upgrades["efficiency"]["amount"] - 1.0: #success
